...
public class GearSubsystem extends Subsystem {
	...
	double rightRange=30;
	double leftRange=26;
	
	public void shiftGearToPeg(){
		if(isBlobThere()){
			if((getTargetCenter()+rightRange)<getPegLocation()){
				slideMotor(-.25);
			}
			else if((getTargetCenter()+leftRange)>getPegLocation()){
				slideMotor(.25);
			}
			else{
	...
	public boolean gearIsAligned(){
		if ((getTargetCenter()+rightRange)>getPegLocation() && (getTargetCenter()-leftRange)<getPegLocation()){
			return true;
		}
		else{
			return false;
